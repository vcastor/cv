...
\definecolor{pagebg}{RGB}{25, 25, 30}
...
\newtcolorbox{qrbox}{
  ...
  overlay={
    ...
    \fill[macyellow] ([xshift=25pt,yshift=-10pt]frame.north west) circle (3pt);
    \fill[macgreen] ([xshift=40pt,yshift=-10pt]frame.north west) circle (3pt);
    \node[white,anchor=center] at ([yshift=-10pt]frame.north) {\footnotesize\ttfamily Website};
  }
}
...
$
$ vcastor.contact = {
~ §\quad\color{white}\faAt§ = victoria.castor-villegas@univ-rouen.fr;
~ §\quad\color{white}\faMapMarker*§ = Rouen, France;
~ §\quad\color{white}\faLinkedinIn§ = §\href{https://www.linkedin.com/in/vcastor}{linkedin.com/in/vcastor}§;
~ §\quad\color{white}\faGithub§ = §\href{https://github.com/vcastor}{github.com/vcastor}§;
~ §\quad\color{white}\faTwitter§ = §\href{https://x.com/vcastorv}{@vcastorv}§;
~ }
\end{macterminal}
% ~ §\quad\color{white}\faPhone*§ = +33 6 47 44 32 65;

\switchcolumn

\begin{qrbox}
  \center
  \vspace{0.5cm}\color{white}\href{https://vcastor.com}{vcastor.com}\\
  \includegraphics[width=0.4\textwidth]{qrs/qr_darkbackground.png}
\end{qrbox}
...
\begin{tikzpicture}[remember picture, overlay, scale=1.0, >=latex, inner sep=0pt, outer sep=2pt,
    mark coordinate/.style={outer sep=0pt, minimum size=3pt, fill=white,circle}]
  ...
  \begin{scope}[shift={(O)}, rotate around={-15:(0,0)},
      field line/.style={color=terminalred, smooth, variable=\t, samples at={0,-5,-10,...,-360}}]
      ...
      \path[xzplane] (Nm) -- +(0,-0.75) coordinate (Nm1) node[below,white] {$\mathbf{N}_m$}
                     (Sm) -- +(0,0.75) coordinate (Sm1) node[above,white] {$\mathbf{S}_m$};
    \draw[very thick, dashed, white] (Sm) -- (Nm);
    \draw[very thick, white] (Sm1) -- (Sm);
    \draw[very thick,->,>=latex', white] (Nm) -- (Nm1);
  \end{scope}
